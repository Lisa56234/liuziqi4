...
int Mamvscomputer::zuijiadain(int hang,int lie){
     ...
     * 每种棋型分为1-5子
     * */
    int b[2][4][5]{{{40,400,3000,5000,10000},{6,10,600,800,10000},{20,120,200,500,0},{6,10,500,700,0}},
                   {{30,300,2500,4000,5000},{2,8,300,600,8000},{26,160,300,0,0},{4,20,300,480,0}}};
    while(fx!=5){
        df1=qixing(fx,hang,lie); fx+=4;
        df2=qixing(fx,hang,lie); fx-=3;   //分别代表两个相反方向的棋型
        int temp;
        if(df1>df2){
        ...
        if(bh1==-1){       //空棋型and其他
            ...
            else  value+=b[x2][y2][z2]+5;continue;
        }
        if(bh1==-2){       //边界冲棋型and其他
            if(bh2<0){value+=0;continue;}
            else value+=b[x2][y2][z2]/2;continue;
        }
        if(bh1==-3){     //边界空冲棋型and其他
            if(bh2<0){value+=0;continue;}
            else value+=b[x2][y2][z2]/3;continue;
        }

        //己活己活，己活己冲，对活对活，对活对冲四种情况赋值
        if(((bh1>=0&&bh1<=4)&&((bh2>-1&&bh2<5)||(bh2>9&&bh2<15)))
                ||((bh1>99&&bh1<105)&&((bh2>99&&bh2<105)||(bh2>109&&bh2<115)))){
            if(z1+z2>4){value+=b[x2][y2][4];continue;}
            else  {value+=b[x2][y2][z1+z2+1];continue;}
        }
        //己冲己冲，对冲对冲情况赋值   因为两边被堵死，如果能凑出六子应赋予极大权值，否则为0
        if(((bh1>9&&bh1<15)&&(bh2>9&&bh2<15))||((bh1>109&&bh1<115)&&(bh2>109&&bh2<115))){
            if(z1+z2>4){value+=10000;continue;}
            else value+=0;continue;
        }
        //己活对活，己活对冲，己冲对活，己冲对冲
        if(((bh1>-1&&bh1<5)&&((bh2>99&&bh2<105)||(bh2>109&&bh2<115)))
                ||((bh1>9&&bh1<15)&&((bh2>99&&bh2<105)||(bh2>109&&bh2<115)))){
            if(z1==4&&z2==4){value+=10000;continue;}
            else{value+=b[x2][y2][z2]+b[x1][y1][z1]/5;continue;}
        }
        else  value+=b[x1][y1][z1]+b[x2][y2][z2];
        continue;
    }
    return value;
}
int Mamvscomputer::qixing(int d,int q1,int q2){
    ...
    fangxiang(d,&q1,&q2);
    if(q1<0||q1>19||q2<0||q2>19) sum=-2;   //边界冲棋
    switch(a[q2+1][q1+1]){
    case jiqi:{
            n++;fangxiang(d,&q1,&q2);
